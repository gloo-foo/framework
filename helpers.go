...
// LineTransform creates a Command that transforms lines one at a time.
...
func (c *lineTransformCommand) Executor() CommandExecutor {
...
func (c *accumulatorCommand) Executor() CommandExecutor {
	return func(ctx context.Context, stdin io.Reader, stdout, stderr io.Writer) error {
		// Collect all lines
		var lines []string
		scanner := bufio.NewScanner(stdin)
...
func (c *accumulateOutputCommand) Executor() CommandExecutor {
	return func(ctx context.Context, stdin io.Reader, stdout, stderr io.Writer) error {
		// Collect all lines
		var lines []string
		scanner := bufio.NewScanner(stdin)
...
func (c *rawCommand) Executor() CommandExecutor {
	return c.fn
}
